...
def binary_classification_loss(concat_true, concat_pred):
    concat_pred = concat_pred.to(torch.device("cpu"))
    concat_true = concat_true.to(torch.device("cpu"))
    
    t_true = concat_true[:, 0].to(torch.device("cpu"))  # 将 t_true 移动到与 concat_pred 相同的设备
    t_pred = concat_pred[:, 2].to(torch.device("cpu"))

    # 确保 t_pred 和 t_true 是浮点型
     # 修正预测值范围，确保在 (0, 1) 内
    
    t_pred = torch.where(torch.isnan(t_pred), torch.tensor(0.5, device=t_pred.device), t_pred)
    t_pred = torch.where(torch.isinf(t_pred), torch.tensor(0.5, device=t_pred.device), t_pred)
    t_pred = torch.clamp(t_pred, min=0.01, max=0.99)
  # 确保值不会超出范围

    # 检查数据类型，确保正确
    if t_true.dtype != torch.float:
        t_true = t_true.float()
    #print("concat_pred shape:", concat_pred.shape)
   # print("t_pred shape:", t_pred.shape)
    #print("t_pred example values:", t_pred[:5])  # 打印前五个值


    # 检查输入合法性
    assert (t_pred > 0).all() and (t_pred < 1).all(), "t_pred contains invalid values"
    assert t_true.dtype == torch.float, "t_true must be a float tensor"

    # 计算二分类交叉熵损失
    loss = F.binary_cross_entropy(t_pred, t_true, reduction='sum')
    return loss
...
def regression_loss(concat_true, concat_pred):
    ...
    t_true = concat_true[:, 0]
    y_true = concat_true[:, 1]
...
def make_tarreg_loss(ratio=1., dragonnet_loss=dragonnet_loss_binarycross):
    ...
    def tarreg_ATE_unbounded_domain_loss(concat_pred, concat_true):
        concat_pred = concat_pred.float()  # 确保预测值为浮点数
        concat_true = concat_true.float()  # 确保真实值为浮点数
        ...
        t_pred = torch.clamp(t_pred, min=1e-6, max=1-1e-6)
...
class DragonNet(nn.Module):
    ...
    def __init__(self, in_features, out_features=[200, 100, 1]):
        ...
        self.representation_block = nn.Sequential(
            nn.Linear(in_features=in_features, out_features=out_features[0]).to(torch.device("cpu")),
            nn.ELU(),
            nn.Linear(in_features=out_features[0], out_features=out_features[0]).to(torch.device("cpu")),
            nn.ELU(),
            nn.Linear(in_features=out_features[0], out_features=out_features[0]).to(torch.device("cpu")),
            nn.ELU()
        )
...
class DragonNet_transfer(DragonNet):
    ...
    def forward(self, x):
        ...
        #for param in self.representation_block.parameters():
            #assert param.device == self.device, "Mismatch in representation_block device"
