...
def run_BDAsplit(data_base_dir='/Users/asus/Desktop/datasets', output_dir='/Users/asus/Desktop/datasets',
    ...
    for idx, simulation_file in enumerate(simulation_files):
        try:
            ...
            # Perform domain adaptation using BDA to get x_t0s_new and x_t1s_new respectively
            x_t0s_new, x_t0t_new = BDA_function(x_t0s, y_t0s, x_t0t, y_t0t, kernel_type='primal', dim=30, lamb=1, mu=0.5, gamma=1, T=10, mode='BDA', estimate_mu_flag=True)
            x_t1s_new, x_t1t_new = BDA_function(x_t1s, y_t1s, x_t1t, y_t1t, kernel_type='primal', dim=30, lamb=1, mu=0.5, gamma=1, T=10, mode='BDA', estimate_mu_flag=True)

            # Merge new source domain data
            Xs_new = np.vstack((x_t0s_new, x_t1s_new))
            Xt_new = np.vstack((x_t0t_new, x_t1t_new))
            ...
            for is_targeted_regularization in [False]:
                ...
                test_outputs, train_output, net, train_dict, test_dict = train_and_predict_dragons(
                    t[target_idx1], y[target_idx1], Xt_new, net, seed=idx, targeted_regularization=is_targeted_regularization,
    ...
    if not os.path.exists(f'./BDA_params_target{target_col_idx}/'):
        os.makedirs(f'./BDA_params_target{target_col_idx}/')
    
    output_file = f'./BDA_params_target{target_col_idx}/BDA_experiments_transfer_{dragon}_{batchsize2}_{l1_reg}_{lr2}.json'
    ...
    print("\n" + "="*30)
    print("BDA (T0T1) Error Summary for All Simulations:")
...
def turn_knob(data_base_dir='/Users/asus/Desktop/datasets/', knob='dragonnet',
    ...
    if knob == 'dragonnet':
        run_BDAsplit(data_base_dir=data_base_dir, output_dir=output_dir, dragon='dragonnet' ,lr2  = lr ,l1_reg = l1reg, batchsize2 = batchsize)

    if knob == 'tarnet':
        run_BDAsplit(data_base_dir=data_base_dir, output_dir=output_dir, dragon='tarnet',lr2  = lr ,l1_reg = l1reg, batchsize2 = batchsize)
